...
export default class CreateCaseDialog extends React.Component {
    ...
    componentDidMount() {
        const interfaceEnabled = SettingsStore.getValueAt(
            SettingLevel.ACCOUNT,
            "ampInterfacesEnabled"
        );
        const username = SettingsStore.getValueAt(
            SettingLevel.DEVICE,
        ...
        if (interfaceEnabled && username !== "") {
            this.setState({
                caseRequesterName: username,
                caseRequesterDisabled: true,
            });
        }
                ...
                for (let i = 0; i < json.length; i++) {
                    json[i].forEach((item) => {
                        if (
                            ...
                            errorObject = {
                                ...errorObject,
                                [item.name]: item.mandatory ? true : false,
                            };
                        }
    ...
    onOk = () => {
        alert(this.state.data.case + " " + this.state.data.patient);
        if (this.state.isFormInvalid) {
            this.setState({ notAllRequiredFields: true });
        }
    ...
    onChange = (group, name, value, type, mandatory) => {
        console.log(value, group, name, type, this.state.errors);
        if (type === "date") {
            value = this.formatDate(value);
        }

        let formErrors = {};
        if (type != "SingleSelect" && type != "Dropdown" && type != undefined) {
            formErrors = this.state.errors;
            formErrors[name] = value == "" && mandatory ? true : false;
            this.setState({
                errors: formErrors,
                isFormInvalid: Object.values(formErrors).some((o) => o),
            });
        }
        ...
        this.setState({
            data: {
                ...this.state.data,
                [group]: { ...this.state.data[group], [name]: value },
            },
        });
    };
    ...
    onAddRecipientClicked = () => {
        const AddressPickerDialog = sdk.getComponent(
    ...
    render() {
        ...
        const importArea = interfaceEnabled ? (
            <InterfaceImport onFinished={this.importData} />
        ) : null;

        const noRecipientSelected = this.state.noRecipientSelected
            ? {}
            : { display: "none" };

        const isFormInvalid = this.state.notAllRequiredFields
            ? {}
            : { display: "none" };
        ...
        return (
            <BaseDialog
                className="amp_CreateCaseDialog"
            ...
            >
                ...
                <form onSubmit={this.onOk}>
                    ...
                    <AdressPicker
                        focus={false}
                        onSelectedListChanged={this.onRecipientChanged}
                        placeholder={_t("Name or AMP.care ID")}
                    />
                    {importArea}
                    {this.parseFormJson()}
                ...
                <div
                    style={noRecipientSelected}
                    className="amp_CreateCaseDialog_error"
                >
                ...
                <div
                    ...
                    className="amp_CreateCaseDialog_error"
                >
                    {_t("Not all required inputs contain data")}
                </div>
                <DialogButtons
                    primaryButton={_t("Send case")}
